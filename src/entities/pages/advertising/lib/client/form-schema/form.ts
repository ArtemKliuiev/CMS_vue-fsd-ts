import {
  ...
  pageAdvertisingSchema,
  PageAdvertisingFormSchema
} from '@/index'
import { ref, nextTick } from 'vue'

export type PageAdvertisingPageFormSchema = ReturnType<typeof usePageAdvertisingForm>

export function usePageAdvertisingForm() {
  const serverErrorMessages = ref<string[]>([])

  const schema = pageAdvertisingSchema()
  const form = useAppForm<PageAdvertisingFormSchema>({
