import {
  ...
  pageVipHallSchema,
  PageVipHallFormSchema
} from '@/index'
import { ref, nextTick } from 'vue'

export type PageVipHallPageFormSchema = ReturnType<typeof usePageVipHallForm>

export function usePageVipHallForm() {
  const serverErrorMessages = ref<string[]>([])

  const schema = pageVipHallSchema()
  const form = useAppForm<PageVipHallFormSchema>({
