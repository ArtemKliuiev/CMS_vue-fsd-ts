import { useAppForm, DetailedErrorMessageItem, pageHomeSchema, PageHomeFormSchema } from '@/index'
import { ref, nextTick } from 'vue'

export type PageHomePageFormSchema = ReturnType<typeof usePageHomeForm>

export function usePageHomeForm() {
  const serverErrorMessages = ref<string[]>([])

  const schema = pageHomeSchema()
  const form = useAppForm<PageHomeFormSchema>({
